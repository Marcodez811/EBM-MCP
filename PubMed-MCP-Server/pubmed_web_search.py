...
def generate_pubmed_search_url(term=None, title=None, author=None, journal=None, 
                               start_date=None, end_date=None, num_results=10, offset=0):
...
def get_pubmed_metadata(pmid):
    ...
    if response.status_code == 200:
        ...
        if article is not None:
            ...
            return None
        doi = None
        pm_data = root.find(".//PubmedData")
        if pm_data is not None:
            article_ids = pm_data.find(".//ArticleIdList")
            if article_ids is not None:
                # xpath syntax: [path@attribute='??']
                doi_element = article_ids.find(".//ArticleId[@IdType='doi']")
...
def search_key_words(key_words, num_results=20, offset=0):
    # 生成搜索 URL
    search_url = generate_pubmed_search_url(term=key_words, num_results=num_results, offset=offset)
...
def search_advanced(term, title, author, journal, start_date, end_date, num_results, offset):
    # 生成搜索 URL
    search_url = generate_pubmed_search_url(term=term, title=title, author=author, 
                                            journal=journal, start_date=start_date, 
                                            end_date=end_date, num_results=num_results, offset=offset)
    print("Generated URL:", search_url)
...
if __name__ == "__main__":
    ...
    # articles = search_key_words("diabetes AND chronic knee pain AND knee replacement surgery AND conservative management", num_results=20, offset=0)
