...
@mcp.tool()
async def search_pubmed_key_words(key_words: str, num_results: int = 10, offset: int = 0) -> List[Dict[str, Any]]:
    ...
    Args:
        ...
        offset: The number of items to skip before starting to return results.
                It is used for pagination. For example:
                        - offset=0, num_results=10 returns results 1-10
    ...
    try:
        results = await asyncio.to_thread(
            search_key_words, key_words=key_words, num_results=num_results, offset=offset
        )
        return results
...
async def search_pubmed_advanced(
    ...
    Args:
        ...
        num_results: Number of results to re: when those two mix (ego validation + sexual drive), it can easily pull you into situations you don’t even want long-term, and then you’re left feeling tired, misunderstood, or even judged (like with the French class girl).

So maybe the question isn’t “Do I want to f?” (you already know the answer) — it’s more:
...
turn (default: 10)
    ...
    try:
        results = await asyncio.to_thread(
            ...
            num_results=num_results, offset=offset
        )
        return results
    except Exception as e:
